...
Reflect.defineMetadata("reducers", [...existingReducers, "artists"], BaseEntity);

export default class ArtistEntity extends BaseEntity {
    
    // SAGA Worker, Side Effect
    private static *fetchArtistsSaga(action: any): Generator<any, void, any> {
        console.log('2. SAGA STARTED', action);
        try {
            yield put({ type: 'artists/FETCH_START' }); // зміна стану loading для роботи з UI
            ...
            const query = stageName ? new URLSearchParams({stageName}).toString() : "";
            const url = query ? `artists?${query}` : `artists`;
            ...
            yield put({ type: 'artists/FETCH_SUCCESS', payload: response }); // --> FETCH_SUCCESS "трігерить" Reducer, який змінює стан на основі отриманих даних
        } catch (error: any) {
            yield put({ type: 'artists/FETCH_ERROR', payload: error.message }); // --> FETCH_ERROR "трігерить" Reducer, який повідомляє про помилку (а "підписники" можуть її прочитати)
        }
    }
    ...
    static {
        this.registerSaga(() => function* () { // передача в BaseEntity функцію-фабрику SAGA
            ...
            yield takeLatest('artists/FETCH_REQUEST', ArtistEntity.fetchArtistsSaga); // у разі запиту tickets/FETCH_REQUEST - виклик fetchTicketsSaga
        });
    }
}
