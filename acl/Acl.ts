...
import { IRoles, IRules } from "./types";
...
export default class Acl implements AclInterface {
    ...
    public addResource(
        ...
        let resourceParent: ResourceInterface | null = null;
        if (parent !== null) {
    ...
    public isAllowed( // чи має ця роль право виконати привілей над ресурсом?
        role: RoleInterface | string,
        resource: ResourceInterface | string,
        privilege: string
    ) {
        // reset role & resource to null
        let allowedRole: RoleInterface; // зберігає роль 
        let allowedResource: ResourceInterface|null; // зберігає ресурс 
        // let allowedPrivilege:string = null;
        if (true||role !== null) {//Ivan: added true ||
            allowedRole = // асигнування обʼєкта до allowedRole, щоб працювати з ним, замість отриманого рядка в процесі перевірки
                typeof role === "string"
        ...
        if (privilege === null) { // перевірка всіх привілеїв ролі, якщо не вказано конкретний привілей (isAllowed("admin", "/users", null))
            // query on all privileges
            do { // перевірка спочатку поточного ресурсу, потім батьківського ресурсу, і так далі, поки не дійде до "allResources" псевдо-батька
                const result = this.roleDFSAllPrivileges(
                    allowedRole,
                    allowedResource
                );
                if (role !== null && result !== null) {
                    return result; // правила різні - allow, deny, або null (немає правил) - повертаємо результат
                }
